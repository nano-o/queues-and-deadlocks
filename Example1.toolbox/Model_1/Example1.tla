...
(*                                                                         *)
(* To speed up model-checking, we'll model queues as sets.  Thus the       *)
(* system will behave as if a node were allowed to process a message       *)
...
(*                                                                         *)
(* With 3 nodes and a fully connected network, there is a deadlock even    *)
(* with queues of size 4, but I wasn't patient enough to explore the whole *)
(* state-space with queues of size 5.                                      *)
(*                                                                         *)
        ...
        while (TRUE) {
            either { \* receive a message
                with (m \in queue[self])
                    msg := m;
                call flood(msg); \* continue flooding the message
l4:
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "b45373f9" /\ chksum(tla) = "54436dbc")
...
Spec == /\ Init /\ [][Next]_vars
        /\ \A self \in Node : WF_vars(node(self)) /\ WF_vars(flood(self))
...
\* Last modified Wed Feb 02 17:39:22 PST 2022 by nano
